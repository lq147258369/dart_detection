...
void Intersect(int T, cv::Mat &Hough2DNorm, cv::Mat &VotesNorm);
...
// FIRST THRESHOLDING
	int T1 = 110; //70 as default value
...
//FILLING 2D HOUGH SPACE MATRIX
	...
	Mat Hough2D;
	Hough2D.create(RoSpan,ThetaSpan, CV_64F);
...
//INTERSECTION VOTING
	...
	Intersection_Votes.create(gray_image.size(), Hough2DNorm.type());
	int TH = 150;
	
	Intersect(TH, Hough2DNorm, Intersection_Votes);
...
///////////MAXIMUM VOTES FINDER///////////////
	...
	int Maximum = 0;
	int dart_centre[2];
	float percentage_window=0.25; //A quarter of the columns of the original image
	...
	for (int x = 0; x < HoughImage.rows; x++ )
		{ 
			for (int y = 0; y < HoughImage.cols; y++ )
			{
				...
				{
					if (Intersection_Votes.at<uchar>(x,y)>T2) {
					image.at<Vec3b>(x,y)[1] = Intersection_Votes.at<uchar>(x,y);//Draw the votes in colour image
					image.at<Vec3b>(x,y)[2] = Intersection_Votes.at<uchar>(x,y);
					image.at<Vec3b>(x,y)[3] = Intersection_Votes.at<uchar>(x,y);
					//cout << "Point"  << Point(x ,y) << endl;
						for (int i=x-window_size;i<x+window_size;i++) { //Look for a particular window of the votes above threshold
							for (int j=y-window_size;j<y+window_size;j++) {
								if (Maximum < Intersection_Votes.at<uchar>(i,j)) {
									Maximum = Intersection_Votes.at<uchar>(i,j);
...
double Yxmax = 0;
double x0=0;
double Xxmax=VotesNorm.cols;
int number_of_lines=0;
for (int ro= 0; ro < Hough2DNorm.rows; ro++ ) //ro
		{ 
			for (int theta = 0; theta < Hough2DNorm.cols; theta++ )//Theta
			{

				if (Hough2DNorm.at<uchar>(ro,theta)>T) { // (ro,Theta)
...
for (int ro= 0; ro < Hough2DNorm.rows; ro++ ) //ro
		{ 
			for (int theta = 0; theta < Hough2DNorm.cols; theta++ )//Theta
			{
				...
				if ((abs((double)(theta* M_PI/180)-(double)(M_PI/2)) > (double)(M_PI/9))) {
					...
					y0 = (double)((ro-(double)(Hough2DNorm.rows/2-1))/cos((double)(theta* M_PI/180)));
					Yxmax = (double)(((ro-(double)(Hough2DNorm.rows/2-1)) - (double)(Xxmax*sin((double)(theta* M_PI/180))))/cos((double)(theta* M_PI/180)));

				}
				else {
					y0=0;
					Yxmax=VotesNorm.rows;
					x0 = (double)((ro-(double)(Hough2DNorm.rows/2-1))/sin((double)(theta* M_PI/180)));
					Xxmax = (double)(((ro-(double)(Hough2DNorm.rows/2-1)) - (double)(Yxmax*cos((double)(theta* M_PI/180))))/sin((double)(theta* M_PI/180)));
	...
	//STORING VOTES//////////////////////
	Mat Voting_pool;
	Voting_pool.create(VotesNorm.size(), CV_64F);
	...
	Normalise(Voting_pool,Voting_Pool_Norm,Hough2DNorm);
	VotesNorm = Voting_Pool_Norm;
	/*int T_inter=0;
